...
bricklet_evse_v4.py: EVSE 4.0 plugin
...
Free Software Foundation, Inc., 59 Temple Place - Suite 4.0,
...
from ..evse_v4_tester import EVSEV4Tester

class Plugin(CoMCUBrickletBase):
    TODO_TEXT = u"""\
1. Set EVSE 4.0 Bricklet DIP switches to 32A (1=Off, 2=Off, 3=On, 4=On)
2. Put EVSE 4.0 Bricklet into EVSE tester
3. Press "Flash"
...
7. Remove EVSE 4.0 Bricklet from EVSE tester
8. Go to 1
"""
    ...
    def start(self):
        ...
        self.evse_tester = EVSEV4Tester(log_func=no_log, start_func=self.request_auto_flash)
        try:
            self.evse_tester.setup()
        except Exception as e:
            self.mw.set_tool_status_error("Error: " + str(e) + ". Connect EVSE V4 tester and press restart test")
    ...
    def get_device_identifier(self):
        return 2999 # BrickletEVSEV2.DEVICE_IDENTIFIER
    ...
    def flash_clicked(self):
        ...
        self.evse_tester.initialize()
        self.evse_tester.set_led(0, 0, 255)

        if not self.flash_bricklet(get_bricklet_firmware_filename(BrickletEVSEV2.DEVICE_URL_PART), 1):
    ...
    def start_test(self, retry_allowed, clear_textedit, is_retry):
        ...
        try:
            ...
            test_iterator = evse_v4_test_generator(self.evse_tester, self.mw.offline, self)
...
TEST_LOG_DIRECTORY = os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', '..', '..', '..', '..', 'test-reports', 'evse_v4'))
OFFLINE_TEST_LOG_DIRECTORY = Path.home() / 'evse_v4_test_report'
...
def test_log_commit_and_push(uid):
    commit_message = 'Add test report for EVSE Bricklet 4.0 with UID ' + uid
    test_log_pull()
...
def evse_v4_test_generator(evse_tester, offline, plugin):
    yield('Searching EVSE Bricklet 4.0 and tester')
    evse_tester.setup()
    #evse_tester.set_led(0, 0, 255)
    evse_tester.set_led_animation(0)

    if evse_tester.find_evse():
        yield('... OK')
    else:
        yield("Failed to find EVSE Bricklet 4.0.")
        evse_tester.exit(1)
        return
    ...
    yield('Checking hardware version (expecting 4.0)')
    ...
    p3v3 = evse_tester.get_p3v3()
    yield('Testing +3300 mV')
    if test_value(p3v3, 3300, 0.05):
        yield(' * ... OK ({0} mV)'.format(p3v3))
        data.append(str(p3v3))
    else:
        yield('-----------------> NOT OK {0} mV (expected 3300 mV)'.format(p3v3))
        evse_tester.exit(1)
        return

    p5v  = evse_tester.get_p5v()
    yield('Testing +5000 mV')
    if test_value(p5v, 5000, 0.05):
        yield(' * ... OK ({0} mV)'.format(p5v))
        data.append(str(p5v))
    else:
        yield('-----------------> NOT OK {0} mV (expected 5000 mV)'.format(p5v))
        evse_tester.exit(1)
        return

    p12v = evse_tester.get_p12v()
    yield('Testing +12000 mV')
    if test_value(p12v, 12000, 0.05):
        yield(' * ... OK ({0} mV)'.format(p12v))
        data.append(str(p12v))
    else:
        yield('-----------------> NOT OK {0} mV (expected 12000 mV)'.format(p12v))
        evse_tester.exit(1)
        return

    m12v = evse_tester.get_m12v()
    yield('Testing -12000 mV')
    if test_value(m12v, -12000, 0.05):
        yield(' * ... OK ({0} mV)'.format(m12v))
        data.append(str(m12v))
    else:
        yield('-----------------> NOT OK {0} mV (expected -12000 mV)'.format(m12v))
        evse_tester.exit(1)
        return
    ...
    while True:
        ...
        if test_value(vol_pppe, 469):
            yield(' * ... OK ({0} mV)'.format(vol_pppe))
            break
        else:
            if time.time() - start > 5:
                yield('-----------------> NOT OK {0} mv (expected 469 mV)'.format(vol_pppe))
                evse_tester.exit(1)
                return
        time.sleep(0.1)
        yield(None)
    ...
    while True:
        vol_pppe = evse_tester.get_pp_pe_voltage()
        if test_value(vol_pppe, 1663):
            yield(' * ... OK ({0} mV)'.format(vol_pppe))
            break
        else:
            if time.time() - start > 5:
                yield('-----------------> NOT OK {0} mv (expected 1663 mV)'.format(vol_pppe))
                evse_tester.exit(1)
                return
        time.sleep(0.1)
        yield(None)
    ...
    while True:
        ...
        yield(None)
    """
    # 100 ohm defective in tester (wrong resistor)
    yield(' * 100 Ohm')
    ...
    yield('Setting 2700 Ohm + 14.0 Ohm resistance')
    evse_tester.set_cp_pe_resistor(True, True, False)
    yield('... OK')
    ...
    test_voltages = [-10302, -9698, -9095, -8476, -7871, -7272, -6648, -6047, -5445, -4822, -4224, -3617, -2908, -2396]
